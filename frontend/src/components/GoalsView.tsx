/**
 * Unified Goals View - Goals list and hierarchy in one view
 */
'use client';
...
export function GoalsView({ realtimeEvents }: GoalsViewProps) {
  ...
  const fetchGoals = async () => {
    try {
      // Fetch stats and root goals in parallel
      const [statsRes, rootsRes] = await Promise.all([
        axios.get(getApiUrl('/api/goals/stats')),
        axios.get(getApiUrl('/api/goals/roots')),
      ]);
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="bg-white/80 rounded-xl p-4">
              ...
              <p className="text-2xl font-bold text-gray-600">{stats.byStatus.pending}</p>
            </div>
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg border-2 border-dashed border-gray-300 p-12 text-center">
          ...
          <p className="text-gray-500">
            No goals yet. Start an agent to see goal hierarchies!
          </p>
        </div>
      ) : (
