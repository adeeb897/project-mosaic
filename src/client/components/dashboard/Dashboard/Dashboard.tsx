import React from 'react';
import { useUser } from '../../../contexts/UserContext';
import styles from './Dashboard.module.css';

// SVG Icon Components
const ChatIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" width="100%" height="100%">
    <path d="M20 2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h4l4 4 4-4h4c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-2 12H6v-2h12v2zm0-3H6V9h12v2zm0-3H6V6h12v2z" />
  </svg>
);

const ModuleIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" width="100%" height="100%">
    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
    <path d="M12 6.5L10.5 10h-3l2.5 2-1 3 2.5-1.5L14 15l-1-3 2.5-2h-3L12 6.5z" opacity="0.7" />
  </svg>
);

const ProfileIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" width="100%" height="100%">
    <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
    <circle cx="12" cy="8" r="2" opacity="0.7" />
  </svg>
);

const SettingsIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" width="100%" height="100%">
    <path d="M19.14,12.94c0.04-0.3,0.06-0.61,0.06-0.94c0-0.32-0.02-0.64-0.07-0.94l2.03-1.58c0.18-0.14,0.23-0.41,0.12-0.61 l-1.92-3.32c-0.12-0.22-0.37-0.29-0.59-0.22l-2.39,0.96c-0.5-0.38-1.03-0.7-1.62-0.94L14.4,2.81c-0.04-0.24-0.24-0.41-0.48-0.41 h-3.84c-0.24,0-0.43,0.17-0.47,0.41L9.25,5.35C8.66,5.59,8.12,5.92,7.63,6.29L5.24,5.33c-0.22-0.08-0.47,0-0.59,0.22L2.74,8.87 C2.62,9.08,2.66,9.34,2.86,9.48l2.03,1.58C4.84,11.36,4.82,11.69,4.82,12s0.02,0.64,0.07,0.94l-2.03,1.58 c-0.18,0.14-0.23,0.41-0.12,0.61l1.92,3.32c0.12,0.22,0.37,0.29,0.59,0.22l2.39-0.96c0.5,0.38,1.03,0.7,1.62,0.94l0.36,2.54 c0.05,0.24,0.24,0.41,0.48,0.41h3.84c0.24,0,0.44-0.17,0.47-0.41l0.36-2.54c0.59-0.24,1.13-0.56,1.62-0.94l2.39,0.96 c0.22,0.08,0.47,0,0.59-0.22l1.92-3.32c0.12-0.22,0.07-0.47-0.12-0.61L19.14,12.94z M12,15.6c-1.98,0-3.6-1.62-3.6-3.6 s1.62-3.6,3.6-3.6s3.6,1.62,3.6,3.6S13.98,15.6,12,15.6z" />
  </svg>
);

export const Dashboard: React.FC = () => {
  const { state: userState } = useUser();

  return (
    <div className={`${styles.dashboard} fade-in`}>
      <div className={styles.header}>
        <h1>Welcome to Project Mosaic</h1>
        <p>Hello, {userState.user?.username || 'User'}! Ready to build something amazing?</p>
      </div>

      <div className={styles.grid}>
        <div className={`${styles.card} scale-in`} style={{ animationDelay: '0.1s' }}>
          <div className={styles.cardIcon}>
            <ChatIcon />
          </div>
          <h3>Chat Interface</h3>
          <p>Engage with AI assistants using our advanced chat system with multi-modal support.</p>
          <a href="/chat" className={styles.cardLink}>
            Start Chatting
          </a>
        </div>

        <div className={`${styles.card} scale-in`} style={{ animationDelay: '0.2s' }}>
          <div className={styles.cardIcon}>
            <ModuleIcon />
          </div>
          <h3>Module Management</h3>
          <p>Install, configure, and manage AI modules to extend your assistant's capabilities.</p>
          <a href="/modules" className={styles.cardLink}>
            Manage Modules
          </a>
        </div>

        <div className={`${styles.card} scale-in`} style={{ animationDelay: '0.3s' }}>
          <div className={styles.cardIcon}>
            <ProfileIcon />
          </div>
          <h3>Profile System</h3>
          <p>Create and customize AI profiles with different personalities and configurations.</p>
          <a href="/profiles" className={styles.cardLink}>
            Manage Profiles
          </a>
        </div>

        <div className={`${styles.card} scale-in`} style={{ animationDelay: '0.4s' }}>
          <div className={styles.cardIcon}>
            <SettingsIcon />
          </div>
          <h3>Settings</h3>
          <p>Configure your preferences, themes, and system settings.</p>
          <a href="/settings" className={styles.cardLink}>
            Open Settings
          </a>
        </div>
      </div>

      <div className={styles.stats}>
        <div className={`${styles.statCard} slide-in-left`} style={{ animationDelay: '0.5s' }}>
          <h4>System Status</h4>
          <div className={styles.statusIndicator}>
            <span className={styles.statusDot}></span>
            All systems operational
          </div>
        </div>

        <div className={`${styles.statCard} slide-in-right`} style={{ animationDelay: '0.6s' }}>
          <h4>Quick Stats</h4>
          <ul>
            <li>
              <span>Active Modules</span>
              <span>0</span>
            </li>
            <li>
              <span>Profiles Created</span>
              <span>0</span>
            </li>
            <li>
              <span>Chat Sessions</span>
              <span>1</span>
            </li>
          </ul>
        </div>
      </div>
    </div>
  );
};
